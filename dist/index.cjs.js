...
function getLineOffset(state) {
  if (isInBlockquote(state)) {
    ...
    const offset = blockState.lineOffsets[state.currentLine];
    if (!offset) {
      return 0;
    }
    return offset;
    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
      ...
      if (currentToken.type === 'text' && state.md.linkify.test(currentToken.content)) {
        ...
        for (let ln = 0; ln < links.length; ln++) {
          ...
          token_t.level = level;
          token_t.position = pos + offset;
          token_t.size = token_t.content.length;
          nodes.push(token_t);
...
function heading(state, startLine, endLine, silent) {
  ...
  token_o.position = originalPos;
  token_o.size = pos - originalPos;
  const originalContent = state.src.slice(pos, max);
  const token_i = state.push('inline', '', 0);
  token_i.content = originalContent.trim();
  token_i.map = [startLine, state.line];
  token_i.children = [];
  token_i.position = pos + trimLeftOffset(originalContent);
  token_i.size = max - pos;
  const token_c = state.push('heading_close', 'h' + String(level), -1);
  token_c.markup = '########'.slice(0, level);
  token_c.position = max;
  token_c.size = originalMax - max;
  return true;
}
...
function lheading(state, startLine, endLine /*, silent */) {
  ...
  token_o.map = [startLine, state.line];
  token_o.position = state.bMarks[startLine];
  token_o.size = 0;
  const token_i = state.push('inline', '', 0);
  token_i.content = content;
  token_i.map = [startLine, state.line - 1];
  token_i.children = [];
  token_i.position = state.bMarks[startLine] + trimLeftOffset(originalContent);
  token_i.size = content.length;
  const token_c = state.push('heading_close', 'h' + String(level), -1);
  token_c.markup = String.fromCharCode(marker);
  token_c.position = state.bMarks[state.line - 1];
  token_c.size = state.bMarks[state.line] - state.bMarks[state.line - 1];
  state.parentType = oldParentType;
  return true;
}
...
StateInline.prototype.pushPending = function () {
  ...
  token.size = token.content.length;
  token.position = this.posStart.shift() + getLineOffset(this);
  this.tokens.push(token);
  this.pending = '';
  return token;
};
...
function postProcess(state, delimiters) {
  ...
  for (let i = max - 1; i >= 0; i--) {
    ...
    token_o.content = '';
    token_o.position = startDelim.position;
